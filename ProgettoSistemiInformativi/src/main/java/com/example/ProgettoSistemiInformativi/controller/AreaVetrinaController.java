...
public class AreaVetrinaController {
    ...
    public ResponseEntity<String> savePayment(@RequestBody String items){
        ...
        if(items != null) {
            ...
            StringBuilder alert = new StringBuilder("\"notifications\": ");
            for (int i = 0; i < productList.length; i++) {
                ...
                int cnt=0;
                List<ProdottoInVetrina> inseriti = new ArrayList<>();
                if ((attuale - quantity) <= prodotto.getMin()) {
                    //Notifiica di sistema, pop-up verso il frontend
                    if(!inseriti.contains(prodotto)){
                        alert.append("\"Il prodotto ").append(prodotto.getCodiceBarre()).append(" deve essere rifornito.\n");
                        inseriti.add(prodotto);
                    }
                }
            ...
            try {
                ...
                if(totalAmount != prezzoVendita) {
                    if (!alert.toString().equals("\"notifications\": ")) {
                        confirmationMessage = "{\"message\": \"Promozione riconosciuta.\", \"totalAmount\": \"" + prezzoVendita + "\", " + alert.toString() + "\"}";
                    } else {
                        confirmationMessage = "{\"message\": \"Promozione riconosciuta.\", \"totalAmount\": \"" + prezzoVendita + "\"}";
                    }
                    System.out.println(confirmationMessage);
                    return ResponseEntity.ok(confirmationMessage);
                }else{
                        if (!alert.toString().equals("\"notifications\": ")) {
                            confirmationMessage = "{\"message\": \"Non sono state riconosciute promozioni.\", \"totalAmount\": \"" + prezzoVendita + "\", " + alert.toString() + "\"}";
                        } else {
                            confirmationMessage = "{\"message\": \"Non sono state riconosciute promozioni.\", \"totalAmount\": \"" + prezzoVendita + "\"}";
                        }
                    System.out.println(confirmationMessage);
                    return ResponseEntity.ok(confirmationMessage);
                }
